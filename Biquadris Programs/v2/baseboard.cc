...
bool BaseBoard::getApplyEffect() {return applyEffect; }
...
void BaseBoard::setApplyEffect(bool fx) { applyEffect = true; }
    ...
    for (int i = 0; i < 4; i++) {
      if (placedBlocks[blocks][i].getX() == x && placedBlocks[blocks][i].getY() == y) {
        ...
        for (int j = 0; j < 4; j++) {
          ...
          int level = placedBlocks[blocks][j].getLevelCreated();
...
void BaseBoard::notifyObservers() {
  for (auto &o : observers) o->notifyBoard(*this);
}
...
bool BaseBoard::gameOver() {
  ...
  vector<pair<int, int>> coords = currentBlock->allFill(curX, curY, curRot);
  for (int i = 0; i < 4; i++) {
    int x = get<0>(coords[i]);
    int y = get<1>(coords[i]);
...
void BaseBoard::clearedLines() {
  ...
  for (int i = minY; i <= maxY && linesChecked <= 4; i++) {
    complete = true;
    ...
    if (complete == true) {
      ...
      for (int f = 0; f < width; f++) {
        linePoints += (removePlacedCell(f, i) + 1) * (removePlacedCell(f, i) + 1);
      }
      for (int k = i - 1; k >= 0; k--) {
        for (int r = 0; r < width; r++) {
          board[r][k - 1].setType(board[r][k].getType());
        }
      }
      shiftPlacedBlks(i);
      for (int r = 0; r < width; r++) {
  ...
  if (linesComp > 0) {
    ...
    notifyObservers();
    if (level == 4) { (static_cast<LevelFour*>(currentLvl))->resetInterval(); } // set reset interval to 0
  }

  if (linesComp >= 2) { setApplyEffect(true); }
