  ...
  while (true) {
    ...
    if (command != "drop") {
      while( (command == "" || command == "-2" || command == "-1")) {
        ...
        if (command == "drop" && dropCounter == 0) {command = "-1"; }
      }
    }
    if (command == "right") {
      ...
      if (activeBoard->gameOver()) { gameOver = true; dropCounter = 0; break;} else {
        ...
        int nextBlk;
        (activePlayer == 1) ? nextBlk = p1NxtBlock : nextBlk = p2NxtBlock;
        activeBoard->setNextBlock((activeBoard->getCurrentLevel())->nextBlock( nextBlk ));
                  cout << "drop error2" << endl;
        activeBoard->doMove(0, 0, 0);
        cout << "drop error1" << endl;
      ...
      if (activeBoard->getLevel() == 3 || activeBoard->getLevel() == 4) {
        ...
        if (theFile.good()) {
          ...
          cout << "Error: The file you have specified does not exist.\n";
        }
      }
